...
<form method="POST">
  ...
  　コメント
  　&thinsp;
  　<input type="text" name="comment"  value='<?php if(isset($ed_comment)){echo $ed_comment;}?>' placeholder="コメント"></input>
...
<?php
    ...
    foreach ($results as $row){
        //$rowの中にはテーブルのカラム名が入る
        echo $row['id'].'<br>'.$row['name'].' '.$row['comment'].' '.$row['post_date'].'<br>';
        echo "<br>";
    }
?>
</body>
